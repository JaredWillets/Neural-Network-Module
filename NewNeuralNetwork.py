...
class NeuralNetwork:
        ...
        for layerNumber, layer in enumerate(structure[1:]):
            ...
            for neuronNumber in range(layer):
                ...
                if layerNumber != len(structure)-1:
                    neuron['weights'] = [rand()
                                         for x in range(structure[layerNumber-1]+1)]
                else:
                    neuron['weights'] = [rand()
                                         for x in range(structure[layerNumber-1])]
                neuron['error'] = float()
                neuron['output'] = float()
    ...
    def trainNetwork(self, iterations: int, learnRate: float, dataset, out=True):
                ...
                for layerNumber in range(len(self.network)):
                    ...
                    for neuronNumber in range(len(layer)):
                        ...
                        for inputNumber in range(len(inputs)-1):
                            neuron['weights'][inputNumber] = float(neuron['weights'][inputNumber]) + (
                                float(learnRate)*float(neuron['error'])*float(inputs[inputNumber]))
